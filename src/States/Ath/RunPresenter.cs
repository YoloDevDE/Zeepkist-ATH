...
public class RunPresenter
{
    ...
    private void AddDetailedWelcomeInfo(Message.Builder message)
    {
        message.AddSeperator("<#B336A3>Author Time Hunting</color>").AddBreakSpace().AddLine("<#E0E0E0>Collect Author Medals within the time limit!</color>").AddBreakSpace().AddSeperator("<#B336A3>Settings</color>").AddBreakSpace()
               .AddKeyValue("<#7FDBFF>Duration</color>", $"<#FFFFFF>{TimeSpan.FromMilliseconds(_ctx.Duration).ToFormattedString()}</color>").AddBreakSpace()
               .AddKeyValue("<#FF7A7A>Skip Penalty</color>", $"<#FF4040>{TimeSpan.FromMilliseconds(_ctx.PenaltyTimeInMilliseconds).ToFormattedString()}</color>").AddBreakSpace().AddSeperator("<#50E451>Commands</color>").AddBreakSpace()
    ...
    public string End()
    {
        RunStatistics stats = new RunStatistics(_ctx.Levels);
        ...
        Message.Builder builder = new Message.Builder().ClearLines().AddLine("<#FFD700>Authortime Hunt finished! :party:</color>").AddBreakSpace().AddSeperator("<#B336A3>Result</color>").AddBreakSpace()
                                                       // Medal Stats
                                                       .AddKeyValue("<#64D2FF>Total ATs</color>", $"<#{ColorDefinitions.Author.CTToHexRGB()}>{_ctx.AuthorMedals}</color>").AddBreakSpace()
                                                       .AddKeyValue("<#64D2FF>ATs Oneshotted!</color>", $"<#50E451>{stats.OneShotAuthorTimes}</color>").AddBreakSpace()
                                                       // Attempt Stats
                                                       ...
                                                       .AddKeyValue("<#64D2FF>Attempts/AT</color>", $"<#FFFFFF>{stats.AverageAttemptsPerAuthorTime:F2}</color>").AddBreakSpace()
                                                       // Time Stats
                                                       .AddKeyValue("<#64D2FF>Avg AT</color>", $"<#FFFFFF>{stats.AverageAuthorTime.GetFormattedTime()}</color>").AddBreakSpace()
                                                       .AddKeyValue("<#64D2FF>Time Wasted</color>", $"<#FF5A5A>{stats.TotalTimeWasted.ToFormattedString()}</color>").AddBreakSpace()
                                                       .AddKeyValue("<#64D2FF>Time/AT</color>", $"<#FFFFFF>{stats.AverageTimePerAuthorTime.ToFormattedString()}</color>").AddBreakSpace();
    ...
    public string BrokenLevel(OnlineZeeplevel level)
    {
        ...
        message.ClearLines().AddSeperator("<#FF0000>Broken Level</color>").AddBreakSpace().AddLine("<#FF5555>Oops! This level appears to be broken or unplayable.</color>").AddBreakSpace()
               .AddLine($"<#64D2FF>{level.Name}</color> by <#FFD700>{level.Author}</color>").AddBreakSpace().AddLine("<#FFFFFF>Automatic recovery in progress</color>").AddBreakSpace()
               .AddLine($"<#AAAAAA>Retries left: <#FFFF00>{_ctx.Retries}</color></color>").AddBreakSpace().AddLine("<#AAAAAA>No time penalty will be applied for this broken level</color>");
    ...
    public string LevelSummary()
    {
        ...
        message.AddBreakSpace().AddKeyValue($"{(CurrentLevel.AuthorTimeAcquired ? "<#50E451>Beaten by</color>" : $"<#{ColorDefinitions.YellowSplit.CTToHexRGB()}>Missed by</color>")}", $"<{diffColor}>{diffDisplay}</color>").AddBreakSpace()
               ...
               .AddKeyValue("<#7FDBFF>Total ATs</color>", $"<#{ColorDefinitions.Author.CTToHexRGB()}>{_ctx.AuthorMedals}</color>").AddBreakSpace();
        ...
        message.AddKeyValue("<#7FDBFF>Time left</color>", $"<{timeLeftColor}>{TimeFormatter.FormatDuration((int)_ctx.GetRemainingTime().TotalMilliseconds)}</color>");
        return message.Build().ToString();
    }
}
