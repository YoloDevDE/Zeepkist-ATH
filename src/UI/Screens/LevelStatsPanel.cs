...
public class LevelStatsPanel : IZeepGUIDrawer
{
	...
	private static void DrawIdentity(ImGui gui, LevelStatsView view)
	{
		...
		UiText.Draw(gui, view.Name, Color.Style.Text.LevelName, UiMetrics.Row(gui, TitleSize * 1.2f), text * TitleSize, 0f);
		UiText.Draw(gui, view.ByAuthor, Color.Style.Text.AuthorName, UiMetrics.Row(gui, 0.9f), text * 0.85f, 0f);
		gui.AddSpacing();
	}
	...
	private static void DrawTargets(ImGui gui, LevelStatsView view)
	{
		...
		{
			UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Your Best", "not finished yet", Color.Style.Text.Muted);
			gui.AddSpacing();
			return;
		}

		UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Your Best", view.BestWithDelta, view.PersonalBestColour);
		gui.AddSpacing();
	}
	...
	private static void DrawEffort(ImGui gui, LevelStatsView view)
	{
		...
		UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Attempts", view.Attempts,
			view.AttemptPending ? Color.Style.Status.Close : Color.Style.Text.Default);
		UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Crashes", view.Crashes, Color.Style.Text.Default);
		UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Wheels Lost", view.WheelsLost, Color.Style.Text.Default);
		UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Time Here", view.TimeOnLevel, Color.Style.Text.Default);
		gui.AddSpacing();
	}

	private static void DrawPace(ImGui gui, LevelStatsView view)
	{
		ImRect row = UiMetrics.Row(gui, 1.2f);
	...
	{
		return UiMetrics.WindowHeight(gui, _contentHeight, 14f);
	}
}
