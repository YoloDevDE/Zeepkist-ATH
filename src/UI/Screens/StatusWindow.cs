...
public class StatusWindow : IZeepGUIDrawer
{
	...
	private static void DrawBackends(ImGui gui)
	{
		...
		UiWidgets.Heading(gui, UiMetrics.Row(gui, 0.85f), "BACKENDS");

		DrawHealth(gui, health.GraphQl);
		DrawHealth(gui, health.Gtr);

		if (UiWidgets.Button(gui, UiMetrics.ButtonRow(gui), health.IsChecking ? "Checking..." : "Check again"))
		{
			health.Refresh();
		}
	...
	private static void DrawHealth(ImGui gui, HealthStatus status)
	{
		...
		UiWidgets.Row(gui, rest, status.Name, status.Verdict, DotColour(status));

		UiText.Draw(gui, status.Detail ?? "", Color.Style.Text.Muted, UiMetrics.Row(gui, 0.9f),
	...
	private void DrawLevel(ImGui gui)
	{
		...
		if (level == null)
		{
			UiText.Left(gui, "No hunt running.", Color.Style.Text.Muted, UiMetrics.Row(gui, 1f));
		...
		UiWidgets.Row(gui, UiMetrics.Row(gui, 1f), "Author", level.Author, Color.Style.Text.AuthorName);
	}
