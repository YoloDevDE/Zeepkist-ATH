...
public class ResultsScreen : IZeepGUIDrawer
{
	...
	private static void DrawHeadline(ImGui gui, RunReportView report)
	{
		...
		UiText.Centre(gui, report.Kicker, Color.Style.Text.Muted, Row(gui, 1f), text * 0.9f);
		UiText.Centre(gui, report.PlayerName, Color.Style.Surface.White, Row(gui, HeadlineSize * 1.2f),
			text * HeadlineSize);
		UiText.Centre(gui, report.Headline, Color.Zeepkist.Medal.Author, Row(gui, 1.4f), text * 1.2f);
	...
	private void DrawLevels(ImGui gui, RunReportView report)
	{
		...
		if (levels.Count == 0)
		{
			UiText.Left(gui, "No levels were played.", Color.Style.Text.Muted, Row(gui, 1f));
			return;
		}
	...
	private void DrawLevelDetail(ImGui gui, LevelRow level)
	{
		...
		if (UiWidgets.IconButton(gui, ButtonRow(gui), UiIcon.Restart, "Back to the list", Color.Style.Action.Restart))
		{
			_level = null;
		}
		...
		UiText.Centre(gui, level.Name, Color.Style.Text.LevelName, Row(gui, 1.6f), text * 1.4f);
		UiText.Centre(gui, level.ByAuthor, Color.Style.Text.AuthorName, Row(gui, 1f), text * 0.9f);
		...
		UiWidgets.Row(gui, Row(gui, 1f), "Attempts", level.Attempts, Color.Style.Text.Default);
		UiWidgets.Row(gui, Row(gui, 1f), "Crashes", level.Crashes, Color.Style.Text.Default);
		UiWidgets.Row(gui, Row(gui, 1f), "Wheels Lost", level.WheelsLost, Color.Style.Text.Default);
		UiWidgets.Row(gui, Row(gui, 1f), "Time Spent", level.Duration, Color.Style.Text.Default);
	}
	...
	private static void DrawThumbnail(ImGui gui, LevelRow level)
	{
		...
		if (thumbnail == null)
		{
			gui.Canvas.Rect(box, ColorExtensions.SurfaceColors.Track, box.H * 0.05f);
	...
	private void DrawHistory(ImGui gui, RunReportView report)
	{
		...
		if (history.Count == 0)
		{
			UiText.Left(gui, "No runs recorded yet. This one is the first.", Color.Style.Text.Muted, Row(gui, 1f));
			return;
		}
	...
	private void DrawRunDetail(ImGui gui, RunReportView run)
	{
		...
		if (UiWidgets.IconButton(gui, ButtonRow(gui), UiIcon.Restart, "Back to the history", Color.Style.Action.Restart))
		{
			_run = null;
			return;
		}
		...
		{
			UiText.Left(gui, "This run was recorded before levels were kept.", Color.Style.Text.Muted, Row(gui, 1f));
