...
public class ControlPanel : IZeepGUIDrawer
{
	...
	private static void DrawIdle(ImGui gui, bool starting)
	{
		...
		if (starting)
		{
			if (UiWidgets.IconButton(gui, ButtonRow(gui), UiIcon.Stop, "Stop", Color.Style.Action.Stop))
			{
				AthRequests.Stop();
			}
	...
	private static void DrawGamemodePicker(ImGui gui)
	{
		...
		UiText.Left(gui, registry.Selected.DisplayName, Color.Zeepkist.Medal.Author, Row(gui, 1f));
		UiText.Draw(gui, registry.Selected.Description, Color.Style.Text.Muted, Row(gui, 0.9f),
	...
	private static void DrawSkipSection(ImGui gui, RunHudView view)
	{
		...
		if (UiWidgets.IconButton(gui, ButtonRow(gui), UiIcon.Warning, "Level is Broken", Color.Style.Action.Broken,
			    racing))
		{
	...
	private static void DrawRunSection(ImGui gui, AthStateMachine run, RunHudView view)
	{
		...
		if (UiWidgets.IconButton(gui, ButtonRow(gui), UiIcon.Stop, "Stop Hunt", Color.Style.Action.Stop))
		{
			AthRequests.Stop();
		}
	}
