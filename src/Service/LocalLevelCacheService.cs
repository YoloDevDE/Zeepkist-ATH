...
public class LocalLevelCacheService
{
    ...
    private void ScanLocalPlaylists()
    {
        ...
        {
            Logger.LogWarning("LocalLevelCacheService: PlaylistApi scan failed or empty, falling back to manual file scan.");
    ...
    private bool TryScanViaApi()
    {
        try
        {
            ...
            Logger.LogInfo($"LocalLevelCacheService: PlaylistApi scan complete, {CachedLevelItems.Count} levels from {playlists.Count} playlists.");
            return true;
        }
        catch (Exception ex)
        {
    ...
    private void TryScanViaFileSystem()
    {
        try
        {
            string playlistDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Zeepkist", "Playlists");
            ...
            foreach (string file in files)
            {
                ...
                {
                    Logger.LogWarning($"LocalLevelCacheService: Failed to parse {Path.GetFileName(file)}: {ex.Message}");
                }
            }

            Logger.LogInfo($"LocalLevelCacheService: File system scan complete, parsed {fileCount}/{files.Length} files, {CachedLevelItems.Count} total levels.");
        }
        catch (Exception ex)
        {
