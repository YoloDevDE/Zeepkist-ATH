...
Die Run-Uhr läuft **ausschließlich in `StateAthWaitingForFinish`**. `Enter()`/`Exit()` schreiben Timestamp-Paare in
...
| #       | Befund                                                                                                                                                                                                                                | Ort                                                  |
|---------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|
| **H1**  | `ConsecutiveDuplicateCount` wird **nie zurückgesetzt** — der Zähler ist kumulativ, nicht „consecutive". Drei über den Run verteilte Duplikate beenden den Run.                                                                        | `AthCtx.cs:22`, `StateAthWaitingForExtraLevel.cs:19` |
| **H2**  | `Retries` (Startwert 3) wird **nie dekrementiert**, nur in `MessageBrokenLevel()` angezeigt und in `StateAthWaitingForNextRun` zurückgesetzt. Der Mechanismus existiert nur als Text.                                                 | `AthCtx.cs:20,88`, `StateAthWaitingForNextRun.cs:40` |
...
| **H6**  | `LastRunTime` / `LastRunMedalStatus` / `LastRunMedalWasNew` werden **nie pro Level zurückgesetzt**. `StateAthWaitingForNextRun` bevorzugt `LastRunTime > 0` und kann damit den Wert des Vorlevels anzeigen.                           | `AthCtx.cs:36-38`, `StateAthWaitingForNextRun.cs:30` |
| **H7**  | `_ = AddLevelAsync()` — fire & forget ohne try/catch. Schlägt das Nachladen fehl, ist die Playlist am Ende leer und der Fehler unbeobachtet.                                                                                          | `StateAthWaitingForFirstRun.cs:52`                   |
...
| **M7**    | `async void` in 6 State-Methoden — davon 2 ohne jedes `await` (`StateAthWaitingForFirstRun.Execute`, `StateAthWaitingForRespawn.OnPlayerSpawned`). Exceptions aus `async void` sind in Unity nicht abfangbar.                                                                                                                                                                                                                                                                                                  |
...
| **M10**   | `StateAthWaitingForNextRun.OnPhotoModeEntered() => OnRoundStarted()` — Photo Mode als Rundenstart zu interpretieren startet die Uhr wieder.                                                                                                                                                                                                                                                                                                                                                                    |
...
| **M10** | `StateAthWaitingForNextRun.OnPhotoModeEntered()` delegiert an `OnRoundStarted()` und startet damit die Uhr. `OnRoundStarted` und `OnPhotoModeEntered` sind aber die einzigen beiden Wege aus `StateAthWaitingForNextRun` zurück in einen Lauf. Ob das ein Fehlgriff ist oder der Workaround dafür, dass `RacingApi.RoundStarted` bei einem Respawn *innerhalb* der Runde nicht feuert, lässt sich nur am laufenden Spiel entscheiden. |
